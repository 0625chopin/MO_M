...
- 회차 팀원 외 작업: 사용자가 별도 세션(`CREW2`)으로 `docs/SCHEDULE/SCHEDULE.md`를 직접 갱신했다(아래 git 절 참고). 팀원 4명의 산출물이 아니라 별도 커밋으로 뒀다.
...
- 커밋 3개로 분리했다 — 성격이 다른 변경을 한 커밋에 섞으면 되돌릴 때 통째로 되돌려야 한다:
  ...
  3. **"SCHEDULE 주차별 배치표를 사람 단위로 재작성 (출처 미확인)"** — `docs/SCHEDULE/SCHEDULE.md` 1개 파일. 팀 2열 → 사람 4열 + 가동 인원 열, 완료 마커 ✅·미결 차단 표시 🚧 도입, "4명 전원 가동은 47주 중 9주뿐"이라는 관찰 추가. **커밋 시점에는 작성자를 확인하지 못해 회차 산출물과 섞지 않고 단독 커밋으로 분리했다.** 커밋 직후 사용자가 **본인 작업(`CREW2` 세션)**이라고 확인해 줬다 — 회차 팀원 4명의 산출물이 아니므로 분리 자체는 결과적으로 맞았고, 커밋 제목의 "출처 미확인"만 사후에 해소된 표현이다(제목을 고치려면 push된 히스토리를 다시 써야 해 그대로 둔다).
