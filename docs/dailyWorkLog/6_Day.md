...
- 비고: **이번 회차에서 유일하게 교차검증 이슈 0건**으로 통과했다. `/sample`의 `PostDetail`에 forbidden(RLS 403)·삭제된 글·잠금 규칙 3종을 개별 도메인 오류 항목으로 등록해 D-030 ③을 다른 Task보다 세분화했다
...
## 판정만 하고 고치지 않은 것
- **`CLAUDE.md`의 「테스트계정」 절**: `chopin0625`/`0625chopin`(`qwer1234`)이 "인증 도입 후 사용할 계정"으로 적혀 있는데 실제 Mock 데모 계정은 `login.ts`의 `MOCK_DEMO_ACCOUNTS`(`seo_runs@example.com`/`runrun25` 등)다. **`CLAUDE.md`는 사용자 소유 파일이라 팀이 고치지 않았다** — 코드 쪽에 관계를 명시하는 선에서 멈췄다. 사용자 판단이 필요하다
...
- **운영 발견**: 팀원이 각자 `npm run build`·`rm -rf .next`를 돌리면 레이스 컨디션으로 ENOENT가 난다(CORE가 1회 겪음). 회차 중반부터 **팀원에게는 `tsc`·`eslint`만 허용하고 build는 팀장이 일괄 실행**하도록 지시를 바꿨다. 다음 회차부터 소환 프롬프트에 처음부터 넣는다
...
- `docs/ISSUES.md`: **I-033**(핸들 형식 규칙 미정의) · **I-034**(`BIO_MAX_LENGTH` 잠정값) · **I-035**(`(app)/crews/[crewId]/*`가 인증만 검사하고 크루원 여부는 미검사) 신규 등재, **I-036**(크루 탐색 게스트 차단) 등재 후 해결됨 전이, **I-025**(인증 화면 전면 게이트 방식 미정) 해결됨 전이
